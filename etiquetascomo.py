...
excel_path = r"C:\Users\Umco\OneDrive\Desktop\ETIQUETAS DEPRATI\COMOHOGAR\UMCO S.A.xlsx"
...
def load_data(file_path):
    try:
        ...
        for sheet in sheet_names:
            ...
            if 'CANT' in temp_df.columns:
                df = temp_df[temp_df["CANT"] > 0]
                if df.empty:
                    temp_df['CANT'] = 1
                    df = temp_df
                break
...
required_columns = ["COD", "DESCRIPCION", "REFERENCIAS", "BARRAS", "PVP", "AFILIADO CONTADO", "CANT"]
...
def draw_label(row):
    ...
    draw.text((10, 15), codigo, font=font_small, fill="black")
    draw.text((label_w_px - 120, 15), "#", font=font_small, fill="black")
    draw.text((label_w_px - 85, 15), referencias, font=font_small, fill="black")
    ...
    if not line1 and draw.textbbox((0, 0), descripcion, font=font_small)[2] <= max_desc_width:
        ...
        if not line1:
            line1 = temp_line.strip()
        draw.text((10, 35), line1, font=font_small, fill="black")
        draw.text((10, 50), line2, font=font_small, fill="black")
        desc_y = 50

    draw.text((18, desc_y + 24), "P.V.P.", font=font_tiny2, fill="black")
    draw.text((15, desc_y + 45), pvp_str, font=font_pvp, fill="black")
    draw.text((118, desc_y + 24), "AFILIADO CONTADO", font=font_tiny, fill="black")
    draw.text((130, desc_y + 43), afiliado_str, font=font_afiliado, fill="black")

    # Código de barras EAN-13
    codigo_limpio = re.sub(r"\D", "", barras_1).zfill(12)[:12]
    try:
        ean = EAN13(codigo_limpio, writer=ImageWriter())
